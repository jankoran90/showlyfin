#!/usr/bin/env bash
# CELLULOID (SHW-98) — vyrobí PODEPSANOU ostrou verzi appky „Filmy" na Zenbooku a nahraje ji na náš
# server na VLASTNÍ OTA kanál `filmy` (jellyfin-uploader /api/appupdate/filmy). Klon channel-release.sh.
#
# Použití:  ./scripts/channel-release-filmy.sh "Changelog text pro tuto verzi"
# Verze se bere z app-filmy/build.gradle.kts (versionCode/versionName) — bumpni JI PŘED během.
#
# Předpoklady (Zenbook yellman, ssh -p 2222):
#   ~/.showlyfin-build.env  = export SIGNING_* + TRAKT_*/TMDB_API_KEY  (Filmy sdílí keystore showlyfinu;
...
NOTES="${1:-Nová verze Filmy}"

VCODE=$(grep -oP 'versionCode\s*=\s*\K[0-9]+' "$SRC/app-filmy/build.gradle.kts" | head -1)
VNAME=$(grep -oP 'versionName\s*=\s*"\K[^"]+' "$SRC/app-filmy/build.gradle.kts" | head -1)
echo "==> CHANNEL release: Filmy v$VNAME (versionCode $VCODE)"
...
scp -P 2222 "yellman@localhost:$ZSRC/app-filmy/build/outputs/apk/release/app-filmy-release.apk" "$APK"
SIZE=$(stat -c %s "$APK")

echo "==> 4/4 zapisuju manifest…"
python3 - "$TEMP/filmy_update.json" "$VCODE" "$VNAME" "$NOTES" "$SIZE" <<'PY'
import json, sys
...
cat "$TEMP/filmy_update.json"; echo
echo "==> Ověření živého endpointu:"
curl -s https://upload.jankoran.cz/api/appupdate/filmy; echo
